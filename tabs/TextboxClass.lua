...
function TextBox:init(locationOfTextbox,sizeOfTextbox,initText,hint)
    -- you can accept and set parameters here
    self.x = locationOfTextbox.x
    self.y = locationOfTextbox.y
    self.width = sizeOfTextbox.x
    self.height = sizeOfTextbox.y
...
function TextBox:draw()
    ...
    if (self.text~="" and self.focus==true) then
        ...
        ellipseMode(CENTER)
        strokeWidth(0)
        ellipse( (self.x + self.width - self.height),(self.y*2+self.height)/2,self.height-25)
        fill(240)
        text("x",(self.x + self.width - self.height),(self.y*2+self.height)/2+2)
    end
    popStyle()
end
